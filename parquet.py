...
# --- File Format Detection & Preprocessing ---
s3_client = boto3.client('s3')

print(f"Checking file format for: s3://{s3_bucket}/{source_file_key}")

# Default to JSON 'auto'
copy_format = "FORMAT AS JSON 'auto'"
s3_path = f"s3://{s3_bucket}/{source_file_key}"

if source_file_key.lower().endswith('.parquet'):
    print("Detected Parquet file. Skipping JSON preprocessing.")
    copy_format = "FORMAT AS PARQUET"
else:
    # Assume JSON, try to handle Array -> NDJSON conversion
    try:
        # Read file content
...
# 2. Copy to Staging (Use determined s3_path and copy_format)
copy_staging_sql = f"""
    COPY {staging_table}
    FROM '{s3_path}'
    IAM_ROLE '{role_arn}'
